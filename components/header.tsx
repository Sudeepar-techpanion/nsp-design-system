...
export function Header({ title, description }: HeaderProps) {
  ...
  const userProfile = {
    name: "Harish",
    email: "harish@customeradmin.com",
    role: "Customer Admin",
    organization: "Nimble S2P Solutions",
    ...
    <>
            ...
            <DropdownMenuTrigger asChild>
              <button className="flex items-center space-x-3 cursor-pointer hover:opacity-80 transition-opacity outline-none">
                <Avatar className="h-9 w-9 bg-primary">
                  <AvatarFallback className="bg-primary text-primary-foreground font-semibold text-sm">H</AvatarFallback>
                </Avatar>
                <div className="hidden sm:block text-left">
                  <div className="text-sm font-medium text-gray-900">Harish</div>
                  <div className="text-xs text-gray-500">Customer Admin</div>
                </div>
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-56">
              <div className="px-3 py-3">
                <p className="text-sm font-semibold text-gray-900">Harish</p>
                <p className="text-xs text-gray-500">Customer Admin</p>
                <p className="text-xs text-blue-600">harish@customeradmin.com</p>
              </div>
              <DropdownMenuSeparator />
      ...
      <Dialog open={profileModalOpen} onOpenChange={setProfileModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm font-medium text-gray-900">{userProfile.role}</p>
              </div>

              <div className="space-y-1">
                <Label className="text-xs font-medium text-muted-foreground">Organization</Label>
                <p className="text-sm font-medium text-gray-900">{userProfile.organization}</p>
              </div>

              <div className="space-y-1">
                <Label className="text-xs font-medium text-muted-foreground">Mobile Number</Label>
                <p className="text-sm font-medium text-gray-900">{userProfile.mobile}</p>
              </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setProfileModalOpen(false)} className="bg-transparent">
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={delegateModalOpen} onOpenChange={(open) => !open && handleCloseModal()}>
        <DialogContent className="sm:max-w-[480px]">
          <DialogHeader>
            <DialogTitle className="text-xl font-semibold text-gray-900">Delegate Access</DialogTitle>
          ...
          <div className="space-y-5 py-4">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        toDate && fromDate && toDate < fromDate && "border-red-500"
                      )}
                    >
                      <CalendarIcon className="mr-2 h-4 w-4" />
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      ...
                      selected={toDate}
                      onSelect={setToDate}
                      disabled={(date) => date < new Date(new Date().setHours(0, 0, 0, 0)) || (fromDate ? date < fromDate : false)}
                      initialFocus
                    />
                  </PopoverContent>
                </Popover>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className={cn(assignToError && "border-red-500 focus-visible:ring-red-500")}
              />
              {assignToError && (
                <p className="text-xs text-red-500">{assignToError}</p>
              )}
              <p className="text-xs text-gray-500">Enter the email of the person who will handle your tasks</p>
            </div>
            ...
            <div className="flex items-center space-x-3 pt-2">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-gray-500">Send an email notification to the assigned person</p>
              </div>
            </div>
          </div>

          <DialogFooter className="gap-3 sm:gap-3">
            <Button variant="outline" onClick={handleCloseModal} className="bg-transparent">
              Cancel
            </Button>
